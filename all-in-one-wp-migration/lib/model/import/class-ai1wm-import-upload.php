			...
			case UPLOAD_ERR_OK:
				try {
					...
					if ( isset( $params['chunk'] ) ) {
						...
						if ( $chunk === 0 ) {
							ai1wm_copy( $upload_tmp_name, $archive_path );
						} else {
							// Subsequent chunks: append to the file
							$out = fopen( $archive_path, 'ab' );
							$in  = fopen( $upload_tmp_name, 'rb' );
						...
						// Verify file data only when the last chunk is uploaded
						if ( $chunk >= ( $chunks - 1 ) ) {
							if ( ! ai1wm_is_filedata_supported( $archive_path ) ) {
								throw new Ai1wm_Upload_Exception(
									wp_kses(
										__(
						...
						// Non-chunked upload (original behavior)
						// Verify file data
